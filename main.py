...
def looks_like_it_needs_search(message: str) -> bool:
    """
    Simple heuristic — factual/info-wale sawal jaise lagte hain unhe
    search ki taraf bhej dete hain. Aage chal kar isko behtar bana sakte hain.
    """
    keywords = [
        "kya hai", "kaun hai", "kab hai", "kahan hai", "kitna", "capital",
        "population", "date", "tarikh", "din", "tyohar", "kaun sa", "when",
        "what is", "who is", "where is", "how much", "how many",
    ]
...
def chat(req: ChatRequest, x_api_key: Optional[str] = Header(None)):
    ...
    search_context = None
    sources = []
    if req.allow_web_search and looks_like_it_needs_search(req.message):
